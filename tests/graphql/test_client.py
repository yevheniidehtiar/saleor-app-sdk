...
class TestSaleorGraphQLClient:
    ...
    def test_client_property(self, auth_token, saleor_api_url, mock_httpx_transport, mock_gql_client):
        """Test client property creates a GQL client with correct configuration"""
        with patch("saleor_app_sdk.graphql.client.Client", return_value=mock_gql_client) as mock_client_class:
            client = SaleorGraphQLClient(api_url=saleor_api_url, auth_token=auth_token)
